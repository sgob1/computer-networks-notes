...
\subsection{Avoiding collapse of the network and the concept of fairness}
...
\subsection{Congestion control variants}
...
\subsection{Estimating the data exchange time}
...
Opening a TCP connection surely involves the decision of the starting sequence
...
TCP is a \emph{reliable} protocol. Reliable only means that data are received, in
order, and with no duplicates. However, application layer does not know how many
bytes are received by the other side of the connection. The \texttt{send()} call may
...
Botnets usually exist for \emph{money} reasons.
