...
{\Large\bfseries}{}{0pt}{\medskip\thesection\ \ \textasciitilde \ \ }
\titleformat{\subsection}[display]
{\large\bfseries}{}{0pt}{\medskip\thesubsection\ \ \textasciitilde \ \ }
...
\textbf{sequence number}\marginpar{\small\textsc{Sequence number}}. A sequence number is a label for each transmitted and
...
There are many variables\marginpar{\small\textsc{snd.User}} involved in sequence numbers. \texttt{snd.User} is the
variable carrying the value of the next byte the \textbf{application} will
send. The variable \texttt{snd.Next}\marginpar{\small\textsc{snd.Next}} carries the value of the next byte that
...
A fifth variable\marginpar{\small\textsc{snd.Ack}} is thus needed: \texttt{snd.Ack}, which \emph{points to the
...
Exact\marginpar{\small\textsc{Choosing RTO}} timings of retransmissions are hard to predict. A sender is completely
...
Regarding the maximum number of retransmissions\marginpar{\small\textsc{Max\_Count}}, value of \texttt{MAX\_COUNT}
